...
def format_and_clean_netbios_name(name):
...
def decode_netbios_name(encoded_name):
...
def parse_netbios_record(reader, cur_client, record_type=None):
...
def parse_netbios_header(reader) -> NBNSHeader:
...
def parse_netbios_packet(data, cur_client):
    reader = BytesIO(data)
